...
server <- function(input, output) {
  ...
  datasetInput <- reactive({load("ExAC_nonTCGA_csqfiltered_short.Rdata")
    data <- vv
  ...
  output$summary <- renderPlot({load("ExAC_nonTCGA_csqfiltered_short.Rdata")
    data <- vv
    if (input$gene != "") {
      data <- data[grepl(toupper(input$gene), data$SYMBOL),]
  ...
  output$tablesum <- renderTable({load("ExAC_nonTCGA_csqfiltered_short.Rdata")
    data <- vv
    if (input$gene != "") {
      data <- data[grepl(toupper(input$gene), data$SYMBOL),]
